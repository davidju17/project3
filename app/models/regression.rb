...
class Regression < ActiveRecord::Base
	...
	def evaluate(type)
		begin
			...
			when 'linear'
				get_linear_coeffs
				puts "#{@linear_coeffs[1]}x + #{@linear_coeffs[0]}"

			when 'polynomial'
				equation = String.new
				
				get_polynomial_coeffs(best_degree)
				
				(0...@polynomial_coeffs.length).reverse_each do |i|
					if @polynomial_coeffs[i] != 0
						if i == 0
							equation << "#{@polynomial_coeffs[i]} + "
						elsif i == 1
							equation << "#{@polynomial_coeffs[i]}x + "
						else				
							equation << "#{@polynomial_coeffs[i]}x^#{i} + "
						end
					end
				end
				
				puts equation[0...-3] # To get rid of the last " + " in the string

			when 'exponential'
				get_exp_coeffs
				puts "#{@exp_coeffs[0]}*e^#{@exp_coeffs[1]}x"

			when 'logarithmic'
				get_log_coeffs	
				puts "#{@log_coeffs[1]}*ln(x) + #{@log_coeffs[0]}"
			...
			when 'best_fit' # chooses the regression with the smallest error
				...
				evaluate(errors.key(errors.values.min))
			end	
		rescue Math::DomainError
